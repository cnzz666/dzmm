...
const TARGET_URL = "https://gpt4novel.com"; // 根据 HAR 确定目标站点
...
(function() {
    ...
    function hackAppState() {
        ...
        // 强行锁定 localStorage 中的状态缓存，防止后端拉取覆盖
    ...
    function killCaptcha() {
        ...
        const captchaNodes = document.querySelectorAll('[class*="captcha"], [id*="captcha"], iframe[src*="captcha"]');
        captchaNodes.forEach(node => node.remove());
    }
    ...
    function injectIsland() {
        ...
        if(techState.authed) {
            ...
            shadow.getElementById('save-cfg').onclick = () => {
                ...
                // 强制 vip3 所需最低积分
                if (techState.vipLevel === 3 && techState.credits < 9999999) {
                    techState.credits = 9999999;
                }
            ...
            shadow.getElementById('inject-cookie-btn').onclick = () => {
                ...
                rawCookie.split(';').forEach(item => {
                    ...
                    if(kv.length >= 2) {
                        ...
                        document.cookie = `${key}=${val}; path=/; SameSite=None; Secure; expires=Fri, 31 Dec 2099 23:59:59 GMT`;
                    }
                });
            ...
            shadow.getElementById('auto-guest-btn').onclick = async () => {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        // 尝试备用端点
                        const altRes = await fetch('/api/v1/user/guest', { method: 'POST', headers: { 'x-force-new-guest': '1' } });
                        if (altRes.ok) {
                            alert('通过备用接口成功获取游客账户');
                            location.reload();
                        } else {
    ...
    // 初始化挂载与全局 Fetch 拦截
    injectIsland();
    setInterval(killCaptcha, 1000); // 防人机二次跳出
    ...
    window.fetch = function() {
        const url = typeof arguments[0] === 'string' ? arguments[0].url : arguments[0].url;
        ...
        if (techState.enabled) {
            ...
            headers.set('x-tech-enabled', '1'); // 关键：告诉目标站点这是代理请求
            // 删除敏感 IP 字段
...
export default {
    async fetch(request, env) {
        ...
        proxyHeaders.set('x-tech-enabled', '1'); // 关键：告诉目标站点这是代理请求
        // 移除客户端真实 IP 字段
        ...
        if (rawSetCookies) {
            // 移除 Domain 限制，将其绑定在当前 Cloudflare Worker 的主机名下
            const cleanedCookie = rawSetCookies.replace(/Domain=[^;]+;?/gi, "").replace(/SameSite=Lax/gi, "SameSite=None; Secure");
        ...
        if (contentType.includes("text/html")) {
            ...
            // 替换网页内部所有绝对路径硬编码域名为代理域名，防止点击直接跳转离开
        ...
        if (contentType.includes("application/json")) {
            ...
            if (isOverride) {
                try {
                    ...
                    // 仅对数据模型中的用户核心节点进行安全精准覆写，绝不破坏 next_level_credits 等升级逻辑
...
function patchUserDataSafely(obj, vip, credits, quota) {
    ...
    if (obj.user && typeof obj.user === 'object') {
        ...
        obj.user.quota = quota; // 强制锁死 50 双模型容量
    }

    // 2. 如果包含顶层 user/profile 响应结构
    if (obj.hasOwnProperty('credits') && typeof obj.credits === 'number') {
        obj.credits = credits;
    }
    if (obj.hasOwnProperty('points') && typeof obj.points === 'number') {
        obj.points = credits;
    }
    ...
    // 3. 递归清洗数组或嵌套对象中的数据，但跳过 next_level 等升级计算字段
    for (let key in obj) {
        if (key.includes('next_level') || key.includes('upgrade') || key.includes('price') || key.includes('cost')) {
            continue; // 跳过计费/升级阈值计算字段，解决需要9999999积分升级的问题
        }
